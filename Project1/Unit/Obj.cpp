...
void Obj::Draw(void)
{
	...
	if (_animMap[_animKey][_animFrame].first != IMAGE_ID("íe")[0] && _animMap[_animKey][_animFrame].first != IMAGE_ID("íe")[1])
	{
		DrawRotaGraph(_pos.x, _pos.y, 1.0, _angle, _animMap[_animKey][_animFrame].first, true);
	}
	else
	{
		DrawRotaGraph(_pos.x, _pos.y, 2.0, _angle, _animMap[_animKey][_animFrame].first, true);
	}
...
bool Obj::SetAnim(const ANIM key, AnimVector& data)
{
	...
	/*if (_animMap.find(key) == _animMap.end()) // è„ÇÃèàóùÇ∆àÍèè
	{
